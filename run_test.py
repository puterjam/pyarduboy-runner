...
def setup_luma_driver(arduboy, spi_speed_mhz=8, refresh_hz=152):
    ...
    try:
        ...
        print(f"✓ OLED video driver configured (Custom SPI @ {spi_speed_mhz} MHz, Plane refresh @ {refresh_hz} Hz)")
        return True
...
Examples:
  ...
  python3 run.py game.hex -v pygame --color amber
    ...
    if VIDEO_DRIVER == 'luma':
        if not setup_luma_driver(arduboy, spi_speed_mhz=args.spi_speed, refresh_hz=args.refresh_hz):
            return 1
